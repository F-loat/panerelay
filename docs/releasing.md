...
- [ ] Confirm `release.config.json`, root/package versions, packed dependencies, Extension `version_name`, and inventory identify `0.1.0`.
...
- [ ] Inspect every npm tarball for its intended public files, exact internal `0.1.0` dependency pins, ACP dependency, and absence of tests, workspace ranges, credentials, logs, and signing keys.
...
- [ ] Remove disposable consumers, browser state, and temporary candidates, retaining only the intentionally accepted `.artifacts/panerelay-0.1.0` directory.
