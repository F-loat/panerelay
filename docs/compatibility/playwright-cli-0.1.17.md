...
Setup verifies Playwright CLI 0.1.17 or newer and installs only Panerelay-owned adapter and Skill files. It does not install or replace `playwright-cli`, edit `PATH` or shell startup files, create user-owned `.playwright/cli.config.json`, or set a Playwright default. Use an explicit attach command or user-managed `PLAYWRIGHT_MCP_CDP_ENDPOINT`/`browser.cdpEndpoint` value.
