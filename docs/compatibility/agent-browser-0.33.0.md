...
Real-browser evidence covers active heartbeat, completed page activity, tab activity, direct Provider release, and two consecutive Qoder CLI 1.1.2 browser turns. Each Qoder terminal event was followed by an inactive browser connection, and a separate agent-browser session immediately acquired control. Forced heartbeat expiry, Bridge-restart gaps, and injected failed or denied CDP commands remain automated-test scenarios to avoid perturbing the user's daily browser profile.
