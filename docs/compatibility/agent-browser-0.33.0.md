...
| ACP live-message remount restoration | Automated | Qoder and OpenCode retain at most 1,000 bounded user/completed-assistant messages inside the live Provider process. A fresh Side Panel mount uses that transcript only when ACP load emits no message history; non-empty Provider history remains authoritative. No prompt or reply is added to Extension storage or logs, and reasoning, activities, approvals, images, partial output, Provider restart, browser authorization, and control leases are unaffected. A dedicated real Qoder 1.1.2 close/reopen check remains pending. |
...
| ACP assistant message ordering | Automated | Qoder and OpenCode preserve provider message IDs and emit independent completion events, so commentary and a later answer remain around intervening tool activity. This changes Side Panel presentation only; it does not grant browser authority or alter control leases. |
| Conversation diagnostic record copy | Automated | The provider-neutral Side Panel action copies versioned, ordered JSON from its current normalized Qoder, OpenCode, Codex, or Claude projection. Deterministic serializer and component tests cover panel/load provenance, turn correlation, bounded content-free normalized-event order, streaming state, success/failure feedback, and exclusion of reasoning text, activity output, unrelated page comments, images, setup data, and raw page content. Version 3 also records the latest observed active, authorized, and controlled tab titles/URLs plus content-free control-session and automation activity metadata so visible-tab and Agent-target confusion can be correlated. The explicit action performs no fresh browser read, authorization, control acquisition, persistence, or upload. |
| Panerelay Skill guidance | Automated | New conversations prefer `$panerelay`, tell the Agent to attempt the canonical `npx skills` installation when unavailable, and allow another browser tool only after installation cannot complete and the fallback is disclosed. Existing Provider/adapter registrations supply a cached fast path that skips generic preflight checks before direct invocation; stale hints fall back to targeted diagnostics and never widen browser authorization. |
...
| New-session exact target orientation | Automated | New conversations additionally carry opaque browser/target UUIDs and bounded engine commands. For agent-browser, the compact reserved session satisfies the pinned upstream name limit, selects the exact live browser, and maps the already-authorized target to `t1`; Browser Use and Playwright consume their own pinned mechanisms. The hint is non-authorizing and fails without URL/title fallback. |
...
On 2026-08-05, a targeted existing-Chrome Provider check installed the local candidate without changing browser defaults or authorization. With an already authorized target kept live, agent-browser 0.33.0 accepted the generated 56-character v2 session and exposed that exact target as `t1`; the legacy 90-character v1 value failed without ordinary-session fallback. The test session and discovery participant were both released without page mutation. Because this bounded check did not originate from a newly created Side Panel conversation, the complete injected-context path remains `Automated` rather than `Verified`.
