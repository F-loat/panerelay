...
| `zhihu` | `packages/sites/src/zhihu`; `answer`, `answer-comments`, `answer-detail`, `collection`, `collections`, `comment`, `download`, `favorite`, `follow`, `followers`, `following`, `hot`, `like`, `pins`, `question`, `recommend`, `search`, `user`, `user-answers`, `user-articles`, `whoami` | Public hot-list E2E passed. Login remains interactive; local image/file workflows remain omitted. |
