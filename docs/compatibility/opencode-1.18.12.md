...
- **Automated**: covered by deterministic contract or Extension tests; a dedicated user-facing acceptance scenario has not yet been completed.
...
| Session list, new, and load | Verified | The isolated runtime listed zero initial sessions, returned an opaque ID for `session/new`, and loaded the created session. Panerelay sends an empty `mcpServers` array and preserves the session working directory for later load/resume. After complete chunk assembly, deterministic tests remove only an exact Panerelay context prefix from the first user message shown in Side Panel history. Additional `Automated` coverage retains at most 1,000 bounded user/completed-assistant messages in the live Provider process and uses them only when a later load emits no history; non-empty OpenCode history remains authoritative. |
| Text and reasoning streaming | Verified | A real prompt completed with `end_turn` and emitted bounded agent-message and agent-thought chunks. Deterministic tests cover fragmented and oversized update handling plus preservation of distinct ACP assistant message IDs across interleaved tool activity, including a stable fallback when an ID is omitted. `session/prompt` follows the ACP turn lifetime rather than the short control-request timeout; initialization and session-control calls remain bounded. |
...
| Side Panel diagnostic record copy | Automated | A user-triggered action copies versioned, ordered JSON from the normalized Side Panel projection, including selected provider, conversation, active-turn, panel/load provenance, turn correlation, a bounded content-free normalized-event trace, and the latest observed active/authorized/controlled tab context. Reasoning and activity output use deterministic size metadata; control activities expose only correlation/status metadata. Component tests cover success, failure, empty state, privacy, message/activity order, and tab context. It does not fetch raw ACP events, persist or upload the record, or imply that provider history omitted before normalization can be reconstructed. |
...
The live-message fallback is process-local and is not written to Extension storage or disk. It restores only provider-neutral user and completed-assistant text after a Side Panel remount; reasoning, activities, approvals, images, partial assistant output, and history after a Provider or browser restart remain outside this fallback.
