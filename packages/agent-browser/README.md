...
When more than one Panerelay browser is connected, set the user browser default with `panerelay browser use <registration-id|chrome|edge>` or scope one invocation with `PANERELAY_BROWSER_ID` / `PANERELAY_BROWSER`. An unavailable default or ambiguous choice fails closed. Every launched session stays pinned to its selected browser, including cleanup.
