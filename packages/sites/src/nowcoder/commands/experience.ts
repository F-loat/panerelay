...
export default defineCommand({
  name: 'experience',
  description: 'List NowCoder interview-experience posts.',
  access: 'read',
  args: [
  ...
  examples: ['panerelay nowcoder experience --page 1 --limit 15'],
  async run(context, args) {
    ...
    const body = await new NowCoderClient(context).authenticatedGet(
      `home/tab/content?tabId=818&categoryType=1&pageNo=${page}&pageSize=${limit}`,
    );
    const records = pick(pick(body, 'data'), 'records');
    if (!Array.isArray(records)) throw new Error('nowcoder experience response is malformed');
    return records
      .map((value, index) => {
        ...
        return {
          ...
          id: pick(content, 'uuid') ?? pick(content, 'id') ?? pick(item, 'contentId') ?? '',
        };
      })
