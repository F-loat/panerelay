...
export function parseSetupArgs(argv: string[]): ParsedSetupArgs {
  ...
  for (const argument of localized.argv.slice(optionStart)) {
...
function doctorHint(id: string, hint: string, locale: SupportedLocale): string {
  ...
  if (id === 'agent-browser') return '请安装 agent-browser，然后运行：panerelay setup';
...
export async function main(
  ...
  try {
    ...
    return result.host.agentBrowserPath ? 0 : 1;
  } catch (error) {
