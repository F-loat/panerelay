...
description: Use Browser Use and Browser Harness through Panerelay in the user's existing Chrome session and explicitly authorized tabs. Use when the user asks Browser Use to operate their current browser, reuse its login or extensions, or work in a tab authorized through the Panerelay side panel.
---
...
3. Invoke Browser Use through the private run surface and keep upstream arguments and standard input unchanged:
   ...
   Use the usual Browser Harness helpers such as `new_tab`, `list_tabs`, `page_info`, `wait_for_load`, `cdp`, `js`, `click_at_xy`, `iframe_target`, and `close_tab`. Prefer one cohesive heredoc for one task so shared page state cannot interleave between separate calls.
...
This launcher enters the same `run browser-use` connection path and starts Browser Use 0.13.7 with `--cli-mcp`. Do not replace it with legacy `browser-use --mcp`, a Python-module MCP server, or a bare upstream executable. Do not edit an MCP client's configuration unless the user asks.

## Connection and lifecycle rules

- Extension mode uses a Panerelay-owned Browser Harness runtime and one persistent daemon lane. Normal task completion does not close the daemon or participant; the Panerelay side panel may continue to show it. Do not run `browser-use --reload` merely to clean up a task.
- The user can revoke tab or site authorization at any time. Revocation, transport loss, heartbeat expiry, Extension reload, or Native Host shutdown removes browser authority. Never retry an authorization denial until the user explicitly authorizes again. For transport loss, retry once through the same run surface so Browser Harness can recover its stale private daemon; if it still fails, report the error and run `npx --yes @panerelay/setup doctor --browser-use`.
...
- Sequential commands from different Agents share the same Browser Harness daemon, selected page, tabs, and event state. They are not task-isolated. Avoid interleaving Agent work; use Direct or a separately owned browser when isolation or parallelism is required.
...
- This Skill covers Browser Use CLI and the Browser Harness-backed CLI MCP surface. It does not make arbitrary Python SDK code transparent; SDK applications need an explicit integration and are outside this workflow.
