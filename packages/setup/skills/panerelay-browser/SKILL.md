...
description: Use agent-browser through Panerelay to work in the user's existing Chrome or Edge session and explicitly authorized tabs. Use when the user asks to operate their current supported browser, reuse an existing login or cookies, work through Panerelay or its side panel, or control a tab they authorized.
---
...
- Do not clear or read cookies for the whole browser profile, create isolated browser contexts, or close the Chrome or Edge browser. `tab new`, `tab close`, and closing the current Panerelay Provider participant remain supported.
- Firefox does not expose the CDP relay required by agent-browser. If Firefox is connected, do not retry or fall back to another browser; use Panerelay's Firefox sidebar for Agent conversations and page comments instead.
