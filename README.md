...
Install the single Panerelay Skill into your Agent:
...
- [agent-browser](https://agent-browser.dev/) 0.33.0 or newer
- [Browser Use CLI](https://docs.browser-use.com/open-source/browser-use-cli) 0.13.7 with Browser Harness 0.1.8
- [Playwright CLI](https://github.com/microsoft/playwright-cli) 0.1.17 or newer
...
Setup manages only Panerelay-owned integration files. It does not install the upstream automation tools.

Authorize the intended current tab or all supported tabs in the Extension, then verify the selected integration. For agent-browser:
...
Each command must list only explicitly authorized tabs. Release ends active control without clearing the selected authorization scope.
