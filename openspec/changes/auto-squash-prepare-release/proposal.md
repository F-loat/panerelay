...
- Do not publish npm packages, create stable tags or GitHub Releases, or submit to the Chrome Web Store from `Prepare Release`.
...
- GitHub repository settings must continue to grant the workflow `contents: write` and `pull-requests: write`; all normal branch-protection and merge-queue rules remain authoritative.
- Stable publication remains a separately dispatched workflow and still requires the protected `release` environment and npm trusted publishing configuration.
