...
- Update setup output, doctor guidance, Extension readiness guidance, package documentation, and official website copy so neither automation engine is described as the default adapter.
...
The change affects `@panerelay/setup` argument parsing, lifecycle orchestration, doctor checks, localized output, Provider/Skill installation, Codex/Claude/Qoder launch configuration, the bounded Native Host integration-install request, setup tests, Extension guidance, READMEs, the project website, release assertions, and the accepted setup decisions in RFC-0001. It does not change the Bridge trust boundary, browser authorization, control leases, automation command semantics, or the pinned compatibility classifications for agent-browser 0.33.0 and Browser Use 0.13.7.
