...
- [x] 1.1 Add the vanilla Vite website workspace package, relative production build configuration, TypeScript entry point, and reusable Panerelay brand assets.
- [x] 1.2 Implement the semantic single-page content structure, responsive visual system, product-flow illustration, mobile navigation, focus states, and reduced-motion behavior.
- [x] 1.3 Add progressive setup-command copying with accessible status feedback while preserving complete no-JavaScript content.
- [x] 1.4 Add complete English and idiomatic Simplified Chinese dictionaries, an accessible language switcher, browser-language selection, local preference persistence, translated metadata, and locale-specific typography while preserving English as the no-JavaScript baseline.

## 2. Product and Trust Content

- [x] 2.1 Add the two Panerelay workflows, installation sequence, Chrome Web Store, GitHub, upstream agent-browser, documentation, compatibility, and license calls to action.
- [x] 2.2 Add explicit authorization, visibility, revocation, credential-retention, browser-ownership, agent-browser 0.33.0 baseline, and Edge `Forwarded` language with authoritative links.
- [x] 2.3 Add page metadata, social preview metadata, icons, and static crawler files without external analytics, unlicensed fonts, or unbounded third-party dependencies.

## 3. Delivery and Verification

- [x] 3.1 Add automated source and production-build tests for required content, accessible controls, and repository-subpath asset resolution.
- [x] 3.2 Add the least-privilege GitHub Pages build and deployment workflow for the default branch and manual dispatch.
- [x] 3.3 Run the frozen install, website tests/build, full workspace check, OpenSpec validation, and `git diff --check`.
...
- [x] 4.6 Reframe the website and English/Chinese README journey around Agent side panel and Automation tool integrations, keep Extension-plus-Skill onboarding primary while moving optional setup integration flags into advanced guidance, add Agent-guided natural-language handoffs for agent-browser, Browser Use, and both, and rerun focused and full validation.
...
- [x] 4.8 Add the repository-level `panerelay-browser` Skill for agent-browser, Browser Use, and Playwright CLI; remove the separate website Agent guide in favor of concise Skill-backed prompts; and compact the workflow engine switcher.
- [x] 4.9 Translate the research into a six-step install-first first-screen storyboard whose separate interface states accurately represent Extension installation, repository Skill installation, Agent-guided automation-tool integration, fail-closed tab authorization, observation followed by Agent control, and user release that preserves authorization.
