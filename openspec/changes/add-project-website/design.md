...
2. **Connect Local** — the real missing-Native-Host side-panel hierarchy, benefits, gray `npx --yes @panerelay/setup` command surface, copy action, retry action, and connected result;
3. **Connect Tool** — a neutral Agent conversation with peer agent-browser, browser-use, and combined choices, followed by published-guide inspection, upstream version detection, selected Panerelay integration, and doctor without implying that Panerelay installs the upstream tool;
...
HTML will contain the complete English content, all six walkthrough interfaces, one stable accessible walkthrough description, both engine panels, the base product setup command, all concise Agent handoff prompts, and usable links. Before enhancement, one coherent walkthrough state remains visible and the complete journey remains available to assistive technology. TypeScript handles language selection, mobile navigation, command and prompt copying, direct walkthrough step selection, the GSAP timeline, the engine tab enhancement, and small view-state enhancements. Inactive walkthrough panels are removed from keyboard navigation and the accessibility tree after enhancement. Copy status uses an `aria-live` region, while automatic walkthrough changes do not repeatedly announce. Navigation, language, playback, step, and engine controls remain keyboard-operable.
...
The comparison may advance every six seconds only before visitor interaction. Pointer hover or focus pauses the timer; manual selection permanently stops it for that page load; `prefers-reduced-motion: reduce` prevents it from starting. The setup section keeps the initial human-facing product command engine-neutral and offers Agent handoff choices for agent-browser, Browser Use, or both. Selecting a handoff changes the visible natural-language prompt and the subordinate manual integration command together, while the initial product installation command remains unchanged and all prompts remain present in source HTML.
...
The website and root READMEs will show one manual Panerelay installation: install the Extension, run engine-neutral setup, and authorize a tab. Workflow-specific setup is offered through one checked-in `docs/agent-setup.md` guide, emitted unchanged by the website build at `https://f-loat.github.io/panerelay/agent-setup.md` and fetchable with `curl -fsSL`. The guide covers agent-browser, Browser Use, and both; it tells the Agent to inspect the environment, install or update the selected upstream tool from its official source only when needed, configure the corresponding Panerelay integration, run doctor, verify the authorized connection, and preserve explicit tab authorization. It states clearly that `@panerelay/setup` installs Panerelay-owned integration files but does not install either upstream automation tool.

Website and README handoffs stay deliberately short: they identify the chosen scenario, point to the stable website guide, and tell the Agent to follow it. Environment checks, detailed commands, unrelated-configuration boundaries, user-controlled Extension installation and tab-authorization stops, diagnostics, and acceptance criteria live in the guide rather than being duplicated across presentation copy.
...
Package READMEs still provide technical CLI reference for explicit adapter flags. Root quickstarts briefly mention that `--agent-browser` and `--browser-use` add the corresponding Panerelay integration and can be combined. The website exposes those flags only as a subordinate command synchronized with the selected Agent-guided scenario, not as competing top-level product installation paths. Version copy distinguishes minimum supported versions from the exact verified records. Browser Use text names the setup-managed CLI, additive Skill, and CLI MCP, and keeps arbitrary Python SDK construction outside the claimed transparent path. The existing `docs/compatibility/` files remain the dense evidence source and are linked rather than duplicated.
