...
- Present Agent side panel and Automation tool integrations as the core product model, with agent-browser, Browser Use, and Playwright CLI as peer choices inside the latter.
...
4. paired Agent side-panel and automation-tool integration workflows, with an agent-browser/Browser Use/Playwright CLI comparison inside the latter;
...
On wide desktop viewports, the hero and 75-pixel positioning strip together fill one dynamic viewport; short desktop heights tighten the hero padding, while intermediate and mobile widths stack the two columns and keep their natural height. Localized headline sizing is constrained by the actual copy column so glyphs cannot paint under the product stage. At narrow widths, every hero grid child receives an explicit shrink boundary, actions use the available inline size, and the walkthrough stage clamps to the hero content box. The fix stays inside the first-screen layout instead of globally hiding overflow, so a future oversized child remains detectable. Subtle reveal states may enhance cause and effect, but the design remains legible without animation and disables non-essential motion under `prefers-reduced-motion`.
...
The global header remains product navigation. A compact segmented `tablist` inside the Automation tool integrations introduction switches between agent-browser, Browser Use, and Playwright CLI because these are automation-engine choices, not site-wide modes. It sizes to its labels instead of spanning the full workflow card and omits redundant integration-status sublabels. Playwright uses the same copy/demo/link panel structure as the other integrations, and the former standalone compatibility strip is removed. The no-JavaScript baseline renders all three panels in document order. Enhancement hides inactive panels, maintains `aria-selected`, `tabindex`, and `aria-controls`, and supports arrow-key navigation across all three tabs.

The comparison may advance every six seconds only before visitor interaction. Pointer hover or focus pauses the timer; manual selection permanently stops it for that page load; `prefers-reduced-motion: reduce` prevents it from starting. The setup section keeps the repository Skill installation command engine-independent and offers Agent handoff choices for agent-browser, Browser Use, Playwright CLI, or all supported integrations. Selecting a handoff changes the visible natural-language prompt and the subordinate manual integration command together, while the Skill installation command remains unchanged and all prompts remain present in source HTML.
...
- [Automatic engine and hero-tool rotation can move content while it is being read] → Keep the intervals restrained, hold surrounding geometry stable, pause the comparison on hover/focus, stop it after interaction, and disable all non-essential rotation under reduced motion.
