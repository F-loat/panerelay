...
- Replace the static first-screen illustration with an install-first walkthrough made from product-accurate interface states: Extension installation, repository Skill installation, Agent-guided automation-tool integration, explicit tab authorization, visible Agent work, and user-controlled release that preserves the selected authorization scope. Keep the wide desktop hero and positioning strip within one dynamic viewport without letting localized copy collide with the product stage.
