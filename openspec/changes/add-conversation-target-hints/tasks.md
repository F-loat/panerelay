...
- [ ] 5.2 In an explicitly authorized daily Chrome fixture, verify agent-browser 0.33.0 maps the injected session target to `t1`, preserves other tab behavior, and fails closed after target revocation; release the exact session and remove fixture artifacts.
- [ ] 5.3 In the same bounded fixture, verify Browser Use 0.13.7/Browser Harness 0.1.8 switches by the injected opaque target on its existing shared lane and reports a stale target without spawning another daemon; restore prior lane state and release control.
- [ ] 5.4 In the same bounded fixture, verify Playwright CLI 0.1.17 uses the injected `-s` session and target-scoped attach with the intended page at index `0`, then fails closed after revocation; detach the exact session and remove temporary evidence.
