import type { PageCommentRuntimeAssets } from './page-comments-runtime-assets.js';

export function installPageCommentsRuntime(assets: PageCommentRuntimeAssets): boolean {
  ...
  const FRAME_TOKEN = `page-comment-frame-${Date.now().toString(36)}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
  const STYLE_PROPERTIES = [...assets.styleProperties] as StyleProperty[];
  let propertyLabels = assets.locales.en.properties;
  let uiLabels = assets.locales.en.ui;
  ...
  const createMarker = (
    ...
    button.innerHTML = assets.icons.edit;
  ...
  const openEditor = (target: Element, existing?: CommentRecord) => {
    ...
    settings.innerHTML = assets.icons.settings;
    ...
    const createConfirm = () => {
      ...
      button.innerHTML = assets.icons.confirm;
      return button;
    };
